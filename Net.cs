...
namespace NebliDex
{
	...
	public partial class App : Application
	{
		...
		public class DexConnection
		{
			...
			//0 - Blockhelper connection
			//1 - Electrum node
			//2 - Not used anymore
			//3 - CN connection
			//4 - Validation connection
			//5 - CN blockhelper connection
		...
		public static void AddDNSServers()
		{
			...
			if(testnet_mode == false){
				...
				ETH_ATOMICSWAP_ADDRESS = "0xcfd9c086635cee0357729da68810a747b6bc674a"; //The address to the Atomic Swap Contract on Ethereum Rinkeby blockchain
				...
				//BlockCypher doesn't use public testnets, don't use
					...
					for(int i=0;i<MyList.Count;i++){
						try {
							...
							if(ConnectSync(client,address,target_port,15))
				            {
								...
								if(js["result"] == null){ //Something wrong with this node
									secure_nStream.Close();
									nStream.Close();
				                	client.Close();
				                	//Try next
								...
								foreach (JToken row in js["result"])
								{
									...
									if(ip_value.IndexOf("onion") > -1){continue;} //No Tor addresses
									if(ip_value.IndexOf(":") > -1){continue;} //No IPv6 addresses
									foreach (JToken port in row[2])
									{
										string port_value = port.ToString();
										if(port_value.StartsWith("s")){ //Now using SSL
											nodes_amount++;
			...
			if(File.Exists(App_Path+"/data/cn_list.dat") == false){
				...
				if(DNS_SEED_TYPE == 0){
					...
					if(resp.Length == 0){
						...
						if(dialog == true){
							MessageBox.Show("Unable to connect to DNS Seed");
						}
						return;
					}
					try {
						...
						foreach (JToken row in js["cn_seed"])
						{
							...
							if(blacklisted == false){ //Only add this ip, if not on blacklist
					...
					try {
						...
						if(CN_Nodes_By_IP.Count == 0 && dialog == true){
							Application.Current.Dispatcher.Invoke((Action)(
								() =>
								{
									MessageBox.Show("No Critical Nodes Found!");
								}));
						}
			                ...
			                do{
								...
								if(r_method.Equals("cn.getlist")){
									...
									foreach (JToken row in js["cn.result"])
									{
										...
										bool blacklisted = CheckCNBlacklist(ip_value);
										if(blacklisted == true){ //Skip this CN if on blacklist
											continue;
										}
			...
			for(int i = min;i < max;i++){
				...
				try {
					...
					if(ConnectSync(client,address,port,5))
		            {
						...
						secure_nStream.ReadTimeout = Timeout.Infinite; //Reset the read timeout to its default
						secure_nStream.WriteTimeout = 15000;
		            	DexConnection dex = new DexConnection();
		...
		{
			if(critical_node == false){ //CNs do not load from the file. They get information from each other.
									...
									if(CN_Node_List_Clone.Count == 1){
										...
										Application.Current.Dispatcher.Invoke((Action)(
											() =>
											{
												...
												if(dialog == true && error_ok == true){ //Don't show non-standard error messages
													MessageBox.Show(dex_mes);
												}
											}));
								...
								if(utxo == null){
									...
									if(dialog == true){
										Application.Current.Dispatcher.BeginInvoke((Action)(
											() =>
											{
												MessageBox.Show("No internet connection has been detected");
											}));
									}
									if(critical_node == true){
										cn_network_down = true; //This will force a redownload of all CN data
			...
			if(CN_Node_List_Clone.Count == 0){
				...
				if(dialog == true){
					Application.Current.Dispatcher.Invoke((Action)(
						() =>
						{
							MessageBox.Show("All critical nodes are down");
						}));
				}
				return;
			}
			...
			if(con.contype == 1){
							...
							for(int i=0;i < DexConnectionReqList.Count;i++){
								if(DexConnectionReqList[i].electrum_con == con && DexConnectionReqList[i].electrum_id == resp_id){
									...
									if(DexConnectionReqList[i].requesttype == 1){
										...
										con.blockdata = msg;
										con.blockhandle.Set(); //Unblock waiting threads						
									}else if(DexConnectionReqList[i].requesttype == 5){ //Received transaction response
				...
				try {
					...
					if(method == "cn.reflectip"){
						...
						if(response == 0){
							if(con.version < protocol_min_version){ //This function requires minimum version
						...
						if(response == 0){
							if(con.version < protocol_min_version && con.contype != 4){ //This function requires minimum version if not validating
								con.open = false;return;
							}
						...
						if(response == 0){
							...
							if(con.outgoing == true){ //Client receives from CN for inclusion
						...
						if(response == 0){
							...
							string error_msg = "";
							if(from_cn == false || js["relay.nonce"] == null){ //First connection
						...
						if(response == 0 && con.outgoing == true){
							...
							Window1.PendOrder(js["cn.result"].ToString());
						}
						...
						if(response == 0){
							...
							if(con.outgoing == true){
								//Pend an order on the charts if its visible
								Window1.ShowOrder(js["cn.result"].ToString());
							}else{
								...
								bool shown = Window1.ShowOrder(js["cn.result"].ToString());
					...
					}else if(method == "cn.orderrequestexist"){ //This is a request to a potential validator
						if(response == 0){
							bool from_cn = CheckIfCN(con);
							if(from_cn == false){
								return; //This is not a CN so cannot get any data
							...
							lock(OrderRequestList){
								for(int i=0;i < OrderRequestList.Count;i++){
									if(OrderRequestList[i].order_nonce_ref.Equals(js["cn.order_nonce"].ToString()) == true && OrderRequestList[i].order_stage < 3){
										...
										if(OrderRequestList[i].order_stage < 1){ //The tradeavail hasn't reached this CN yet
					...
					}else if(method == "cn.trade_complete"){ //Show the completed trade to TNs
						if(response == 0){
							//Both the TN and CNs and receive this message from a critical node
							if(con.outgoing == true){ //Client receives from CN for inclusion
								//We received this from a CN (more trusted), evaluate the relayed order message
								if(critical_node == false){
									...
									Window1.AddRecentTradeToView(market,type,trade_price,trade_amount,order_nonce,trade_time);
								}
							}
						}
					}else if(method == "cn.validator_taker_canceltrade"){ //Taker receives this when maker rejects the trade / trade timeouts
						if(response == 0){
							//Both the TN and CNs and receive this message from a critical node
							if(con.outgoing == true && con.contype == 4 && critical_node == false){ //From validation node
								string order_nonce = js["cn.order_nonce"].ToString();
					...
					}else if(method == "cn.validator_maker_canceltrade"){ //Maker receives message when taker fails to fund account
						if(response == 0){
							//Both the TN and CNs and receive this message from a critical node
							if(con.outgoing == true && con.contype == 4 && critical_node == false){ //From validation node
								string order_nonce = js["cn.order_nonce"].ToString();
					...
					}else if(method == "cn.validator_trade_reject"){ //Validator receives this from a trader
						if(response == 0){
							if(con.outgoing == false && con.contype == 4 && critical_node == true){ //From validation node
		...
		public static bool RemoveElectrumServer(int type,string ip){
			...
			lock(DexConnectionList){ //Wait till this list is available
				try {
		...
		public static void RecreateCNList(){
			...
			lock(CN_Nodes_By_IP){ //Make this code thread safe
				try {
			        using (System.IO.StreamWriter file_out = 
		...
		public static void PeriodicNetworkQuery(object state)
		{
			...
			try {
				...
				for(int i = 0;i < total_markets;i++){
					lock(RecentTradeList[i]){
						for(int pos=RecentTradeList[i].Count-1;pos >= 0;pos--){
							if(UTCTime() - RecentTradeList[i][pos].utctime > 60*60*24){ //Remove after 24 hours
				...
				lock(DexConnectionList){
					for(int i=DexConnectionList.Count-1;i >= 0;i--){
						...
						if(mydex.open == false){
							...
							if(mydex.outgoing == false){ //Inbound connection
					...
					lock(OpenOrderList[market]){
						for(int pos=OpenOrderList[market].Count-1;pos >= 0 ;pos--){
							if(OpenOrderList[market][pos].order_stage > 0){
								if(UTCTime() - OpenOrderList[market][pos].pendtime > max_transaction_wait){ //Pended for over 3 hours
				...
				lock(DexConnectionList){
					for(int i=0;i < DexConnectionList.Count;i++){
						...
						try {
							if(mydex.contype == 1){
								if(ctime-mydex.lasttouchtime > 50){ //Send keep alive every 50 seconds
								...
								if(mydex.outgoing == true){
									...
									if(ctime-mydex.lasttouchtime > 90){ //Cut off connection after 90 seconds of no signal
									...
									if(mydex.version == 0 && mydex.contype == 3){
										...
										if(ctime-mydex.lasttouchtime > 20){ //Cut off connection after 20 seconds
								...
								if(mydex.outgoing == true){
									if(ctime-mydex.lasttouchtime > 70){ //Send keep alive every 70 seconds
				...
				if(main_window_loaded == true && Application.Current != null){
					Application.Current.Dispatcher.BeginInvoke((Action)(
						() =>
						{
			...
			try {
				//Reset the system counter to prevent the system from sleeping
				//For windows only
				NativeMethods.ResetSystemTimer();
			} catch (Exception) {
				NebliDexNetLog("Failure to reset system clock");
			}
						...
						if(CN_Nodes_By_IP.Count > 1){
							...
							() =>
							{
								...
								if(run_headless == false){
									MessageBox.Show("Critical node out of sync. Must manually reactivate node.","Notice",MessageBoxButton.OK,MessageBoxImage.Error,MessageBoxResult.OK,MessageBoxOptions.DefaultDesktopOnly);
									if(main_window_loaded == true){
									main_window.Title = "NebliDex: A Decentralized Neblio Exchange "+version_text;
									main_window.Menu_Item_CN.IsChecked = false;
									main_window.CN_Tab.Visibility = Visibility.Collapsed;
									main_window.CN_Menu_Clear.Visibility = Visibility.Collapsed;
								}
								}else{
					...
					if(MyOpenOrderList[i].queued_order == true){
						if(UTCTime() - MyOpenOrderList[i].pendtime > 30){
							...
							if(remove_order == true){
								...
								{
									if(main_window_loaded == true){
										main_window.Open_Orders_List.Items.Refresh();
									}										
								}));
							}else{
		...
		public static void GetCNMarketData(int market)
		{
			...
			try {
				...
				NebliDexNetLog("Error retreiving market data: "+e.ToString());
			}
		...
		public static void CheckElectrumServerSync()
		{
			...
			for(int etype = 1; etype < total_cointypes;etype++){
				...
				try {
					string blockdata="";
					lock(dex.blockhandle){ //Prevents other threads from doing blocking calls to connection
						dex.blockhandle.Reset();
		...
		public static bool MakerReceiveValidatorInfo(DexConnection con, JObject js)
		{
			...
			try {
				...
				if(ConnectSync(client,address,critical_node_port,30))
	            {
					...
					if(trade_amount > myord.amount || trade_amount < min_amount){ //The amount requested cannot be greater than openorder amount
						TraderRejectTrade(dex,order_nonce,reqtime,true); //Send message to cancel trade
						nStream.Close();
						client.Close();
					...
					if(Math.Abs(UTCTime() - adjusted_ctime) > 60*3){ //Contract time difference greater than 3 minutes, not allowed
