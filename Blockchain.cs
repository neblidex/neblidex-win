	...
	public partial class App : Application
	{
			...
			try {
				...
				if(wallet_ntp1 == false && sendtoken == false){
					...
					foreach (JToken row in utxo)
					{
						...
						string tokid = row["tx_tokenid"].ToString();
						if(tokid.Length == 0){ //Pure Neblio, add to the inputs
							OutPoint utxout = OutPoint.Parse(tx_hash+"-"+tx_pos); //Create the Outpoint
							TxIn tx_in = new TxIn();
							tx_in.PrevOut = utxout;
				...
				if(satoshi_amount >= dust_output){ //If less than dust, do not send
					if(to_address != null){
						tout = new TxOut()
						{
				...
				if(satoshi_change > dust_output){ //Send change back
					tout = new TxOut()
					{
		...
		public static Transaction CreateNTP1AllTokenTransfer(string to_add)
		{
			...
			try {
				...
				//Now we determine where to send the coins
				if(satoshi_send_amount >= dust_output){ //May not have a to_address if less than dust
					tout = new TxOut()
					{
		...
		public static void AddMyRecentTrade(int market, int ordertype, decimal price, decimal amount, string txhash, int pending)
		{
			...
			Application.Current.Dispatcher.BeginInvoke((Action)(
				() =>
				{
					...
					main_window.Trade_History_List.Items.Insert(0,new MyTrade{ Date = format_date, Pair = format_market, Type = format_type, Price = String.Format(CultureInfo.InvariantCulture,"{0:0.########}",price), Amount = String.Format(CultureInfo.InvariantCulture,"{0:0.########}",amount), TxID = txhash } );
					main_window.Trade_History_List.Items.Refresh();
				}));
		}
		...
		public static void UpdateMyRecentTrade(string txhash, int pending)
		{
			...
			Application.Current.Dispatcher.BeginInvoke((Action)(
				() =>
				{
					int rows = main_window.Trade_History_List.Items.Count;
					for(int i = 0;i < rows;i++){
						//Go through all the rows and find the one that matches this hash
						MyTrade my_recent = (MyTrade)main_window.Trade_History_List.Items.GetItemAt(i);
						if(my_recent.TxID == txhash){
							//Remove this row and insert new row at top
							main_window.Trade_History_List.Items.Remove(my_recent);
							main_window.Trade_History_List.Items.Insert(0,new MyTrade{ Date = format_date, Pair = my_recent.Pair, Type = my_recent.Type, Price = my_recent.Price, Amount = my_recent.Amount, TxID = my_recent.TxID } );
							break;
						}
					}
					//Now refresh it
		...
		public static void AddMyCNFee(int market,decimal fee)
		{
			...
			Application.Current.Dispatcher.BeginInvoke((Action)(
				...
				{
					//Put this recent trade at the beginning
					main_window.CN_Tx_List.Items.Insert(0,new { Date = format_date, Pair = format_market, Fee = String.Format(CultureInfo.InvariantCulture,"{0:0.########}",fee) } );
					main_window.CN_Tx_List.Items.Refresh();
				}));
		}
		...
		public static void ClearAllCNFees()
		{
			...
			Application.Current.Dispatcher.BeginInvoke((Action)(
				...
				{
					//Put this recent trade at the beginning
					main_window.CN_Tx_List.Items.Clear();
					main_window.CN_Tx_List.Items.Refresh();
				}));
		}
				...
				try {
					...
					if(type == 0){
						...
						if(UTCTime() - tx_waittime > 30){ //Been more than 30 seconds since checked this transaction
							...
							myquery = "Update MYTRANSACTIONS Set waittime = @time Where nindex = @index;";
							if(maker_txconf == 0){ //Maker transaction hasn't confirmed yet (probably not sent yet)
								if(UTCTime() > canceltime){
									...
									if(maker_txhex.Length == 0){ //Only if maker hasn't tried to send transation before
									...
									if(tx != null || eth_tx != null){
										...
										if(broadcast_ok == true){
											...
											if(myord != null){
												...
												Window1.ShowOrder(myord.order_nonce);
											}
										}
									}									
								}
						...
						if(UTCTime() - tx_waittime > 30){ //Been more than 30 seconds since checked this transaction
							...
							if(conf >= 1){
								...
								//Not confirmed
								int txtime = Convert.ToInt32(table.Rows[i]["utctime"].ToString()); //Time of transaction
								if(UTCTime() - txtime > max_transaction_wait && conf == 0){ //3 hours of being unconfirmed
					...
					if(status == 0){
						...
						if(UTCTime() - row_utctime > max_transaction_wait){
							...
							if(contract_balance == 0){
								...
								Window1.ShowOrder(table.Rows[i]["order_nonce_ref"].ToString());
			...
			try {
				...
				for(int i = 0;i < WalletList.Count;i++){
					...
					if(unconfirmed_exist == false){
						...
						if(status != 2){ //Not waiting status
			...
			if(connectiontype != 0){
				...
				try {
					...
					foreach (JToken row in js["result"])
					{
						...
						if(height > 0){ //If the height is 0, the utxo is unconfirmed
				...
				try {
					...
					foreach (JToken row in result["utxos"])
					{
						...
						if(height >= 0){ //Do not look at unconfirmed tx
							JObject line = new JObject();
		...
		public static int TransactionConfirmations(int cointype, string txhash)
		{
			...
			if(connectiontype != 0){
				...
				try {
					string blockdata="";
					lock(con.blockhandle){ //Prevents other threads from doing blocking calls to connection
						con.blockhandle.Reset();
