...
namespace NebliDex
{
	...
	public partial class App : Application
	{
		...
		public static string getPublicFacingIP()
		{
			...
			if(DNS_SEED_TYPE == 0){ //HTTP IP lookup
				bool timeout;
		...
		public static void ToggleCNStatus(Intro win)
		{
			...
			if(critical_node == false){
					...
					System.Windows.Application.Current.Dispatcher.BeginInvoke((Action)(
						() =>
						{
							...
							if(run_headless == false){
								MessageBox.Show(win,"You do not have enough NDEX to become a Critical Node ("+cn_ndex_minimum+" NDEX)");
								CloseCNStatusWindow(win);
							}else{
								Console.WriteLine("You do not have enough NDEX to become a Critical Node ("+cn_ndex_minimum+" NDEX)");
							}
						}));
					return;
				}
				...
				if(my_ip == ""){
					...
					() =>
					{
						//Must be on UI thread
						if(run_headless == false){
							MessageBox.Show(win,"Could not resolve your Public IP address");
							CloseCNStatusWindow(win);
						}else{
				...
				if(critical_node == false){
					...
					System.Windows.Application.Current.Dispatcher.BeginInvoke((Action)(
						() =>
						{
							//Must be on UI thread
							if(run_headless == false){
								MessageBox.Show(win,"The Critical Node port is already occupied. Port: "+critical_node_port);
								CloseCNStatusWindow(win);
							}else{
				...
				if(firstnode == true){
					...
					System.Windows.Application.Current.Dispatcher.BeginInvoke((Action)(
						() =>
						{
							//Must be on UI thread
							if(run_headless == false){
								MessageBox.Show(win,"You are the first Critical Node on the network.\nPlease do not close the program when validating transactions.");
								CloseCNStatusWindow(win);
							}else{
				...
				if(blockdata == ""){
					...
					System.Windows.Application.Current.Dispatcher.BeginInvoke((Action)(
						() =>
						{
							//Must be on UI thread
							if(run_headless == false){
								MessageBox.Show(win,"Could not connect to CN");
								CloseCNStatusWindow(win);
							}else{
				...
				if(blockdata != "CN Accepted"){
					...
					System.Windows.Application.Current.Dispatcher.BeginInvoke((Action)(
						() =>
						{
							...
							if(run_headless == false){
								if(error_ok == true){
									MessageBox.Show(win,"Your Critical Node status request has been rejected by the Critical Node\n"+blockdata);
								}
								CloseCNStatusWindow(win);
							}else{
								if(error_ok == true){
				...
				Window1.ClearMarketData(exchange_market); //Remove the current chart data
				...
				if(critical_node_pending == true){
					...
					System.Windows.Application.Current.Dispatcher.BeginInvoke((Action)(
						() =>
						{
							//Must be on UI thread
							if(run_headless == false){
								MessageBox.Show(win,"Your Critical Node status request was rejected because the connection was interrupted. Please try again.");
								CloseCNStatusWindow(win);
							}else{
				...
				System.Windows.Application.Current.Dispatcher.BeginInvoke((Action)(
					() =>
					{
						//Must be on UI thread
						if(run_headless == false){
							MessageBox.Show(win,"You are now a Critical Node.\nPlease do not close the program when validating transactions.");
							CloseCNStatusWindow(win);
						}else{
				...
				if(cn_num_validating_tx > 0){
					...
					System.Windows.Application.Current.Dispatcher.BeginInvoke((Action)(
						...
						{
							//Must be on UI thread
							MessageBox.Show(win,"You are still validating transactions. Please wait before closing Critical Node.");
							CloseCNStatusWindow(win);
						}));
					return;
				}
		...
		public static void RebroadcastCNStatus()
		{
			...
			if(blockdata != "CN Accepted"){
				...
				System.Windows.Application.Current.Dispatcher.BeginInvoke((Action)(
					() =>
					{
						//Must be on UI thread
						if(run_headless == false){
							MessageBox.Show("Could not re-establish your critical node status.","Notice",MessageBoxButton.OK,MessageBoxImage.Error,MessageBoxResult.OK,MessageBoxOptions.DefaultDesktopOnly);
							if(main_window_loaded == true){
								...
								main_window.Export_CN_Fee.Visibility = Visibility.Collapsed;
							}
						}else{
			...
			if(cn_network_down == false){
				...
				Window1.ClearMarketData(-1); //Remove all market data
		...
		public static JObject CNValidateTxFees(JObject tx, DexConnection con,bool from_maker)
		{
			...
			int stale_order_time = 0;
			
			lock(OrderRequestList){
				for(int i = 0;i < OrderRequestList.Count;i++){
					if(OrderRequestList[i].order_nonce_ref.Equals(order_nonce) == true && OrderRequestList[i].order_stage > 0){
						if(OrderRequestList[i].order_stage == 3){ //Closed already
							possibly_stale = true;
							stale_order_time = OrderRequestList[i].utctime;
							continue;
						}
			...
			if(req == null){
				//Could not find an active trade for this order
				if(possibly_stale == true){
					//Order was open but now it is stale, let trader know
					NebliDexNetLog("Trade already closed, sending notice to trader: "+order_nonce);
					if(from_maker == false){
						SendPostTradeAction(0,stale_order_time,order_nonce); //Tell taker trade is already canceled
					}else{
						SendPostTradeAction(1,stale_order_time,order_nonce); //Tell maker trade is already canceled
					}
				}
				return null;
			}
